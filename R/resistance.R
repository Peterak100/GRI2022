...
prepare_resistance_files <- function(taxa, taxapath) {
  ...
  for (i in 1:nrow(taxa)) {
    taxon <- taxa[i, ]
    tryCatch({
      crop_filename <- file.path(taxon_path(taxon, taxapath),
      ...
      warning("Failed to download resistance file for ",
              taxon$ala_search_term, " ", e)
    })
  }
  return()
}
...
download_hdm <- function(taxon, taxapath, crop_filename) {
  ...
  url <- paste0("https://maps2.biodiversity.vic.gov.au/Models/SMP_",
                taxon_escaped, "_", common_name, "_", taxon_id, ".zip")
